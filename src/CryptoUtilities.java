import components.naturalnumber.NaturalNumber;
import components.naturalnumber.NaturalNumber2;
import components.random.Random;
import components.random.Random1L;
import components.simplereader.SimpleReader;
import components.simplereader.SimpleReader1L;
import components.simplewriter.SimpleWriter;
import components.simplewriter.SimpleWriter1L;

/**
 * Utilities that could be used with RSA cryptosystems.
 *
 * @author Nicholas McCracken
 *
 */
public final class CryptoUtilities {

    /**
     * Private constructor so this utility class cannot be instantiated.
     */
    private CryptoUtilities() {
    }

    /**
     * Useful constant, not a magic number: 3.
     */
    private static final int THREE = 3;

    /**
     * Pseudo-random number generator.
     */
    private static final Random GENERATOR = new Random1L();

    /**
     * Returns a random number uniformly distributed in the interval [0, n].
     *
     * @param n
     *            top end of interval
     * @return random number in interval
     * @requires n > 0
     * @ensures <pre>
     * randomNumber = [a random number uniformly distributed in [0, n]]
     * </pre>
     */
    public static NaturalNumber randomNumber(NaturalNumber n) {
        assert !n.isZero() : "Violation of: n > 0";
        final int base = 10;
        NaturalNumber result;
        int d = n.divideBy10();
        if (n.isZero()) {
            /*
             * Incoming n has only one digit and it is d, so generate a random
             * number uniformly distributed in [0, d]
             */
            int x = (int) ((d + 1) * GENERATOR.nextDouble());
            result = new NaturalNumber2(x);
            n.multiplyBy10(d);
        } else {
            /*
             * Incoming n has more than one digit, so generate a random number
             * (NaturalNumber) uniformly distributed in [0, n], and another
             * (int) uniformly distributed in [0, 9] (i.e., a random digit)
             */
            result = randomNumber(n);
            int lastDigit = (int) (base * GENERATOR.nextDouble());
            result.multiplyBy10(lastDigit);
            n.multiplyBy10(d);
            if (result.compareTo(n) > 0) {
                /*
                 * In this case, we need to try again because generated number
                 * is greater than n; the recursive call's argument is not
                 * "smaller" than the incoming value of n, but this recursive
                 * call has no more than a 90% chance of being made (and for
                 * large n, far less than that), so the probability of
                 * termination is 1
                 */
                result = randomNumber(n);
            }
        }
        return result;
    }

    /**
     * Finds the greatest common divisor of n and m.
     *
     * @param n
     *            one number
     * @param m
     *            the other number
     * @updates n
     * @clears m
     * @ensures n = [greatest common divisor of #n and #m]
     */
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        /*
         * While m is not zero, use Euclid's algorithm where GCD(n, m) = GCD( m,
         * n mod m) to find greatest common divisor. If m is zero, greatest
         * common divisor has been found, which is n.
         */
        if (!m.isZero()) {
            /*
             * Create new natural numbers to modify and transfer into the
             * original n and m to ensure the original n and m are referencing
             * the correct values.
             */
            NaturalNumber mCopy = new NaturalNumber2(m);
            NaturalNumber nModM = new NaturalNumber2(n.divide(m));
            n.transferFrom(mCopy);
            m.transferFrom(nModM);

            // Recurse using modified n and m values by Euclid's algorithm.
            reduceToGCD(n, m);
        }
    }

    /**
     * Reports whether n is even.
     *
     * @param n
     *            the number to be checked
     * @return true if n is even
     * @ensures isEven = (n mod 2 = 0)
     */
    public static boolean isEven(NaturalNumber n) {
        /*
         * Check if the last digit of n is even, which means the entire
         * NaturalNumber must be even as well.
         */
        int digit = n.divideBy10();
        n.multiplyBy10(digit);
        return digit % 2 == 0;
    }

    /**
     * Updates n to its p-th power modulo m.
     *
     * @param n
     *            number to be raised to a power
     * @param p
     *            the power
     * @param m
     *            the modulus
     * @updates n
     * @requires m > 1
     * @ensures n = #n ^ (p) mod m
     */
    public static void powerMod(NaturalNumber n, NaturalNumber p,
            NaturalNumber m) {
        assert m.compareTo(new NaturalNumber2(1)) > 0 : "Violation of: m > 1";

        /*
         * If the power is not zero, perform the operations for the
         * fast-powering algorithm using recursive calls of the function and
         * immediately following every multiplication of n by reducing modulo m.
         */
        if (!p.isZero()) {
            if (isEven(p)) {
                /*
                 * For even powers, half of p must be used for the recursive
                 * call of the fast-powering algorithm.
                 */
                p.divide(new NaturalNumber2(2));

                // For even powers, perform a recursive call with half of p.
                powerMod(n, p, m);

                // Multiply n to the power of half of p by itself.
                NaturalNumber nCopy = new NaturalNumber2(n);
                n.multiply(nCopy);

                // Restore p to it's previous value.
                p.multiply(new NaturalNumber2(2));
            } else {
                /*
                 * For odd powers, p - 1 must be used for the recursive call of
                 * the fast-powering algorithm.
                 */
                p.decrement();

                // Store a copy of n before the value is updated.
                NaturalNumber nCopy = new NaturalNumber2(n);

                // For odd powers, perform a recursive call with p - 1.
                powerMod(n, p, m);

                /*
                 * Multiply the updated n value to the power of p - 1 by it's
                 * non-updated value.
                 */
                n.multiply(nCopy);

                // Restore p to it's previous value.
                p.increment();
            }
            /*
             * Reduce the result of multiplication of n modulo m no matter what
             * the parity of p is.
             */
            n.transferFrom(new NaturalNumber2(n.divide(m)));
        } else {
            /*
             * If the power is currently zero, set the number to be raised to a
             * power to one since a number raised to zero will always equal one.
             */
            n.transferFrom(new NaturalNumber2(1));
        }
    }

    /**
     * Reports whether w is a "witness" that n is composite, in the sense that
     * either it is a square root of 1 (mod n), or it fails to satisfy the
     * criterion for primality from Fermat's theorem.
     *
     * @param w
     *            witness candidate
     * @param n
     *            number being checked
     * @return true if w is a "witness" that n is composite
     * @requires n > 2 and 1 < w < n - 1
     * @ensures <pre>
     * isWitnessToCompositeness =
     *     (w ^ 2 mod n = 1)  or  (w ^ (n-1) mod n /= 1)
     * </pre>
     */
    public static boolean isWitnessToCompositeness(NaturalNumber w,
            NaturalNumber n) {
        assert n.compareTo(new NaturalNumber2(2)) > 0 : "Violation of: n > 2";
        assert (new NaturalNumber2(1)).compareTo(w) < 0 : "Violation of: 1 < w";
        n.decrement();
        assert w.compareTo(n) < 0 : "Violation of: w < n - 1";
        n.increment();

        /*
         * Initialize boolean to be false, meaning if neither witness to
         * compositeness check is met then the boolean will not be modified and
         * return false.
         */
        boolean isComposite = false;

        // Create a copy of w to ensure w is restored (not modified).
        NaturalNumber wCopy = new NaturalNumber2(w);
        /*
         * Update the copy of w in accordance to the first compositeness check
         * in the ensures clause.
         */
        powerMod(wCopy, new NaturalNumber2(2), n);

        /*
         * If the updated copy of w is equal to 1, then w is a witness that n is
         * composite, therefore skip to the return statement since only one
         * compositeness check must be true for w to be a witness that n is
         * composite.
         */
        if (wCopy.equals(new NaturalNumber2(1))) {
            isComposite = true;
            // If the first compositeness check is false, then check the second.
        } else {
            // Reset the modified copy of w back to the original value of w.
            wCopy.copyFrom(w);
            // Create a copy of n to ensure n is restored (not modified).
            NaturalNumber nCopy = new NaturalNumber2(n);
            // Set the copy of n to equal n - 1.
            nCopy.decrement();

            /*
             * Update the copy of w in accordance to the second compositeness
             * check in the ensures clause.
             */
            powerMod(wCopy, nCopy, n);

            /*
             * If the updated copy of w does not equal 1, then w is a witness
             * that n is composite.
             */
            if (!wCopy.equals(new NaturalNumber2(1))) {
                isComposite = true;
            }
        }
        return isComposite;
    }

    /**
     * Reports whether n is a prime; may be wrong with "low" probability.
     *
     * @param n
     *            number to be checked
     * @return true means n is very likely prime; false means n is definitely
     *         composite
     * @requires n > 1
     * @ensures <pre>
     * isPrime1 = [n is a prime number, with small probability of error
     *         if it is reported to be prime, and no chance of error if it is
     *         reported to be composite]
     * </pre>
     */
    public static boolean isPrime1(NaturalNumber n) {
        assert n.compareTo(new NaturalNumber2(1)) > 0 : "Violation of: n > 1";
        boolean isPrime;
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            /*
             * 2 and 3 are primes
             */
            isPrime = true;
        } else if (isEven(n)) {
            /*
             * evens are composite
             */
            isPrime = false;
        } else {
            /*
             * odd n >= 5: simply check whether 2 is a witness that n is
             * composite (which works surprisingly well :-)
             */
            isPrime = !isWitnessToCompositeness(new NaturalNumber2(2), n);
        }
        return isPrime;
    }

    /**
     * Reports whether n is a prime; may be wrong with "low" probability.
     *
     * @param n
     *            number to be checked
     * @return true means n is very likely prime; false means n is definitely
     *         composite
     * @requires n > 1
     * @ensures <pre>
     * isPrime2 = [n is a prime number, with small probability of error
     *         if it is reported to be prime, and no chance of error if it is
     *         reported to be composite]
     * </pre>
     */
    public static boolean isPrime2(NaturalNumber n) {
        assert n.compareTo(new NaturalNumber2(1)) > 0 : "Violation of: n > 1";

        boolean isPrime;

        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            // Two and three are primes.
            isPrime = true;
        } else if (isEven(n)) {
            // Evens are always composite.
            isPrime = false;
        } else {
            /*
             * Used to count the number of witness canditates still left to be
             * generated.
             */
            int witnessCanditatesLeft = 50;

            // Create a copy of n to ensure n is restored (not modified).
            NaturalNumber nCopy = new NaturalNumber2(n);
            // Set the copy of n to equal n - 2 by subtracting 2.
            nCopy.subtract(new NaturalNumber2(2));

            // Create a NaturalNumber to store the current witness being tested.
            NaturalNumber currentWitness = new NaturalNumber2();

            // Presume isPrime is true to allow while loop to run.
            isPrime = true;

            /*
             * Generate and test 50 witness canditates unless one is found to be
             * a witness that n is composite.
             */
            while (isPrime && 0 < witnessCanditatesLeft) {
                /*
                 * Set the current witness to be tested to a NaturalNumber in
                 * the range of 0 and n - 2 inclusive.
                 */
                currentWitness
                        .transferFrom(new NaturalNumber2(randomNumber(nCopy)));
                /*
                 * If the random number generated is greater than or equal to
                 * two, then the witness is within the acceptable range for the
                 * contract of isWitnessToCompositeness, therefore call the
                 * function.
                 */
                if (currentWitness.compareTo(new NaturalNumber2(2)) >= 0) {
                    /*
                     * If the currently tested witness is a witness that n is
                     * composite, then isPrime must be false.
                     */
                    isPrime = !isWitnessToCompositeness(currentWitness, n);
                }
                witnessCanditatesLeft--;
            }
        }
        return isPrime;
    }

    /**
     * Generates a likely prime number at least as large as some given number.
     *
     * @param n
     *            minimum value of likely prime
     * @updates n
     * @requires n > 1
     * @ensures n >= #n and [n is very likely a prime number]
     */
    public static void generateNextLikelyPrime(NaturalNumber n) {
        assert n.compareTo(new NaturalNumber2(1)) > 0 : "Violation of: n > 1";

        /*
         * Make n odd if it is even, since even numbers greater than three
         * cannot be prime since two will always be factor.
         */
        if (isEven(n) && n.compareTo(new NaturalNumber2(THREE)) >= 0) {
            n.increment();
        }

        /*
         * Start at n and increase through odd numbers until one is likely prime
         * with a low probability of error.
         */
        while (!isPrime2(n)) {
            n.add(new NaturalNumber2(2));
        }
    }

    /**
     * Main method.
     *
     * @param args
     *            the command line arguments
     */
    public static void main(String[] args) {
        SimpleReader in = new SimpleReader1L();
        SimpleWriter out = new SimpleWriter1L();

        /*
         * Sanity check of randomNumber method -- just so everyone can see how
         * it might be "tested"
         */
        final int testValue = 17;
        final int testSamples = 100000;
        NaturalNumber test = new NaturalNumber2(testValue);
        int[] count = new int[testValue + 1];
        for (int i = 0; i < count.length; i++) {
            count[i] = 0;
        }
        for (int i = 0; i < testSamples; i++) {
            NaturalNumber rn = randomNumber(test);
            assert rn.compareTo(test) <= 0 : "Help!";
            count[rn.toInt()]++;
        }
        for (int i = 0; i < count.length; i++) {
            out.println("count[" + i + "] = " + count[i]);
        }
        out.println("  expected value = "
                + (double) testSamples / (double) (testValue + 1));

        /*
         * Check user-supplied numbers for primality, and if a number is not
         * prime, find the next likely prime after it
         */
        while (true) {
            out.print("n = ");
            NaturalNumber n = new NaturalNumber2(in.nextLine());
            if (n.compareTo(new NaturalNumber2(2)) < 0) {
                out.println("Bye!");
                break;
            } else {
                if (isPrime1(n)) {
                    out.println(n + " is probably a prime number"
                            + " according to isPrime1.");
                } else {
                    out.println(n + " is a composite number"
                            + " according to isPrime1.");
                }
                if (isPrime2(n)) {
                    out.println(n + " is probably a prime number"
                            + " according to isPrime2.");
                } else {
                    out.println(n + " is a composite number"
                            + " according to isPrime2.");
                    generateNextLikelyPrime(n);
                    out.println("  next likely prime is " + n);
                }
            }
        }

        /*
         * Close input and output streams
         */
        in.close();
        out.close();
    }

}
